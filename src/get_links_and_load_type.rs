...
pub fn get_links_and_load_type<R: TryFrom<Entry>>(
    ...
    let entries: Vec<(Record, ActionHash, Link)> =
        super::get_latest_records(link_info, GetOptions::default())?;
    Ok(entries
        .iter()
        .flat_map(|(entry, hash, link)| {
            if let RecordEntry::Present(entry) = &entry.entry {
                match R::try_from(entry.clone()) {
                    Ok(e) => Ok((e, hash.clone(), link.clone())),
